...
def make_averaged(original_function, total_samples=1000):
    ...
    def make_averaged_1(*args):
        ...
        while total_samples>=number:
            total+=original_function(*args)
            number+=1
        number-=1
        return total/number
    return make_averaged_1
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    #这里出现了一个错误，浪费了不少时间
    #每次roll，他的结果都会累积，每次只能roll一次
    averaged_dice=make_averaged(roll_dice,total_samples)
    num_rolls=1
    number=2
    result=1
    another_val=averaged_dice(number,dice)
    max_val=averaged_dice(num_rolls,dice)
    while number<=10:
        if another_val>max_val:
            max_val=another_val
            result=number
            num_rolls=number
        else:
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    score+=b
    c=hog_pile(score, opponent_score)
    if c>0:
        return 0
    return num_rolls
